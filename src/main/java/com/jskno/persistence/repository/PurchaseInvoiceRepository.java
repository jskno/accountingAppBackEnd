...
public interface PurchaseInvoiceRepository extends JpaRepository<PurchaseInvoice, Long> {
    ...
    @Query("SELECT d FROM PurchaseInvoice d WHERE d.invoiceNumber = :invoiceNumber")
    List<PurchaseInvoice> getPurchaseInvoiceByInvoiceNumber(
            @Param("invoiceNumber") String invoiceNumber);

    @Query("SELECT d FROM PurchaseInvoice d WHERE d.totalInvoice = :totalInvoice and d.company = :company"
            + " and d.date = :date")
    List<PurchaseInvoice> getPurchaseInvoiceByTotalInvoiceCompanyAndDate(
            @Param("totalInvoice") Double invoiceNumber,
